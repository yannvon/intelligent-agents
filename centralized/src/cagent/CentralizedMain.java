...
public class CentralizedMain implements CentralizedBehavior {
    ...
    public List<Plan> plan(List<Vehicle> vehicles, TaskSet tasks) {
        ...
        for (int vId = 0; vId < best.length; vId++) {
            ...
            while (next != null) {
                ...
                for (City city : current.pathTo(nextCity)) { //FIXME no need to do this ?
    ...
    private double computeCost(ActionEntry[] actions, List<Vehicle> vehicles) {
        double sum = 0;
        int i = 0;
        double max = 0; // FIXME I don't think we should take max here, but sum all (its okey if one car does most of work)
    ...
    private static class ActionEntry {
        ...
        public double cost(City lastPos) {
            ...
            return lastPos.distanceTo(nextCity) + (next == null ? 0 : next.cost(nextCity)); // FIXME Yann: check this
        }
         ...
         *
         * @param maxLoad the maximum load of the vehicle
